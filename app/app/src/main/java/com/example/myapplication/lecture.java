...
public class lecture extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        rv_lecture = rootView.findViewById(R.id.rv_lecture);
        mtaskitemList = new ArrayList<>();

        fragmentManager = getActivity().getSupportFragmentManager();

        Cursor c = MainActivity.DB.read_lecture();
        if(c.getCount() != 0) {
            while(c.moveToNext()){
        ...
        rv_lecture.setLayoutManager(new GridLayoutManager(getContext(), 1));
        rv_lecture.setAdapter(mRecyclerViewAdapter);
